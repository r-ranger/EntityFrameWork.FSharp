...
module Extensions =
    ...
    type ModelBuilder with
        ...
        member this.UseValueConverterForType
            (
                ``type`` : Type,
        ...
        member this.RegisterOptionTypes() =
            ...
            let makeOptionConverter t =
                ...
                let converter =
                    converterType.GetConstructor([||]).Invoke([||])
                    :?> ValueConverter
            ...
            let converterDetails =
                this.Model.GetEntityTypes()
                |> Seq.filter(fun p ->
                    not <| SharedTypeExtensions.isOptionType p.ClrType
                )
                |> Seq.collect(_.ClrType.GetProperties())
                |> Seq.filter(fun p ->
        ...
        member this.RegisterSingleUnionCases() =
            let makeSingleUnionCaseConverter tUnion =
                ...
                let converterType =
                    genericSingleCaseUnionConverterType.MakeGenericType(
                        underlyingType,
                        tUnion
                    )
            ...
            let converterDetails =
                this.Model.GetEntityTypes()
                |> Seq.filter(fun p ->
                    not <| SharedTypeExtensions.isSingleCaseUnion p.ClrType
                )
                |> Seq.collect(_.ClrType.GetProperties())
                |> Seq.filter(fun p ->
                    SharedTypeExtensions.isSingleCaseUnion p.PropertyType
                )
                |> Seq.map(fun p ->
                    (p, (makeSingleUnionCaseConverter p.PropertyType))
                )
