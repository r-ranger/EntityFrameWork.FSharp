...
module internal rec SharedTypeExtensions =
    ...
    let processArrayType
        ...
        processType innerType useFullName compilable sb |> ignore

        while innerType.IsArray do
            sb.Append('[').Append(',', innerType.GetArrayRank() - 1).Append(']')
            |> ignore
