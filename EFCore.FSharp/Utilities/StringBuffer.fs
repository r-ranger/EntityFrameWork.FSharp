...
module StringBuffer =
    ...
    type StringBufferBuilder() =
        ...
        member inline _.Yield(c : char) =
            fun (b : IndentedStringBuilder) ->
                Printf.kprintf (b.AppendLine >> ignore) "%c" c
        ...
        member inline _.Yield(txt : string option) =
            fun (b : IndentedStringBuilder) ->
                match txt with
                | Some t -> Printf.kprintf (b.AppendLines >> ignore) "%s" t
                | None -> ()

        member inline _.Yield(txt : string option seq) =
            fun (b : IndentedStringBuilder) ->
                for tt in txt do
                    match tt with
                    | Some t -> Printf.kprintf (b.AppendLines >> ignore) "%s" t
    ...
    let stringBuilder = StringBufferBuilder()
