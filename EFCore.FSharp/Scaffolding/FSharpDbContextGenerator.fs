...
type FSharpDbContextGenerator
    ...
    let generateForeignKeyConfigurationLines
        ...
        let annotations =
            annotationCodeGenerator.FilterIgnoredAnnotations(
                foreignKey.GetAnnotations()
            )
            |> annotationsToDictionary

        lines.Add
            $"(fun (%s{identifier}: Builders.EntityTypeBuilder<_>) -> %s{identifier}.HasOne<%s{targetType}>().WithMany()"
        ...
        if not(foreignKey.PrincipalKey.IsPrimaryKey()) then
            ...
            lines.Add $".HasPrincipalKey(%s{principalKeyProps})"
        ...
        lines.Add $".HasForeignKey(%s{fkProps})"
    ...
    let generateSequence (s : ISequence) =
        ...
        let methodName =
            if s.ClrType = Sequence.DefaultClrType then
                ...
                $"HasSequence<%s{FSharpUtilities.getTypeName(s.ClrType)}>"

        let parameters =
            if
                (s.Schema |> String.IsNullOrEmpty)
                && (s.Model.GetDefaultSchema() <> s.Schema)
            then
                $"%s{s.Name |> FSharpUtilities.delimitString}, %s{s.Schema |> FSharpUtilities.delimitString}"
            else
        ...
        stringBuilder {
            ...
            indent {
                ...
                if
                    s.MinValue <> Nullable(Sequence.DefaultMinValue |> int64)
                then
                    $".HasMin({s.MinValue})"

                if
                    s.MaxValue <> Nullable(Sequence.DefaultMaxValue |> int64)
                then
                    $".HasMax({s.MaxValue})"
    ...
    let generateLambdaToKey
        ...
        | 0 -> ""
        | 1 ->
            $"fun %s{lambdaIdentifier} -> %s{lambdaIdentifier}.%s{properties[0].Name} :> obj"
        | _ ->
            let props =
                properties
                |> Seq.map(fun p -> $"%s{lambdaIdentifier}.%s{p.Name}")
                |> join ", "

            $"fun %s{lambdaIdentifier} -> (%s{props}) :> obj"

    let generatePropertyNameArray (properties : IReadOnlyList<IProperty>) =

        let props =
            properties |> Seq.map(fun p -> code.Literal p.Name) |> join "; "
        ...
        if isNull key then
            ...
            if earlyExit then
                ...
                if explicitName then
                    lines.Add $".HasName(%s{code.Literal(key.GetName())})"
        ...
        if explicitTable then
            ...
            let parameterString =
                if explicitSchema then
                    $"%s{code.Literal tableName}, %s{code.Literal schema}"
                else
    ...
    let generateProperty (property : IProperty) useDataAnnotations =
        ...
        let lines = ResizeArray<string>()
        lines.Add $".Property(fun e -> e.%s{property.Name})"
        ...
        if useDataAnnotations then
            ...
            annotationCodeGenerator.GenerateDataAnnotationAttributes(
                property,
                annotations
            )
            |> ignore
        ...
        | true ->
            lines.Add(
                $".HasColumnType({code.Literal(property.GetConfiguredColumnType())})"
            )
        ...
        match
            property.GetPrecision() |> Option.ofNullable,
            property.GetScale() |> Option.ofNullable
        with
        ...
        | :? IConventionProperty as cp ->
            match
                cp.GetValueGeneratedConfigurationSource() |> Option.ofNullable
            with
            | Some valueGeneratedConfigurationSource when
                ...
                let methodName =
                    ...
                    | ValueGenerated.OnAddOrUpdate when
                    ...
                    | _ ->
                        invalidOp
                            $"Unhandled enum value ValueGenerated.{valueGenerated}"
    ...
    let generateRelationship (fk : IForeignKey) useDataAnnotations =
        ...
        lines.Add(
            sprintf
                ...
                (if isNull fk.DependentToPrincipal then
                     ...
                     $"fun d -> d.%s{fk.DependentToPrincipal.Name}")
        )
        ...
        lines.Add(
            sprintf
                ...
                (if isNull fk.PrincipalToDependent then
                     ...
                     $"fun p -> p.%s{fk.PrincipalToDependent.Name}")
        )
        ...
        if not(fk.PrincipalKey.IsPrimaryKey()) then
            ...
            let typeParam =
                if fk.IsUnique then
                    $"<%s{(fk.PrincipalEntityType :> ITypeBase).DisplayName()}>"
                else
                    ""

            let methodParams = code.Lambda(fk.PrincipalKey.Properties, "p")

            lines.Add $".HasPrincipalKey%s{typeParam}(%s{methodParams})"

        let typeParam =
            if fk.IsUnique then
                $"<%s{fk.DeclaringEntityType.Name}>"
            else
                ""

        let methodParams =
            fk.Properties |> Seq.map(fun p -> "d." + p.Name) |> join ", "

        lines.Add
            $".HasForeignKey%s{typeParam}(fun (d:%s{fk.DeclaringEntityType.Name}) -> (%s{methodParams}) :> obj)"
        ...
        if fk.DeleteBehavior <> defaultOnDeleteAction then
            canUseDataAnnotations <- false
            lines.Add $".OnDelete(%s{code.Literal fk.DeleteBehavior})"

        if
            not(String.IsNullOrEmpty(string fk[RelationalAnnotationNames.Name]))
        then
            canUseDataAnnotations <- false
    ...
    let generateManyToMany (skipNavigation : ISkipNavigation) =
        ...
        let createJoinEntity (joinEntityType : IEntityType) =
            ...
            let generateManyToManyIndex (index : IIndex) =
                ...
                let indexProps =
                    index.Properties |> Seq.map(_.Name) |> Seq.toArray
            ...
            let indexes =
                joinEntityType.GetIndexes() |> Seq.map generateManyToManyIndex
            ...
            let generateManyToManyProperties (property : IProperty) =
                ...
                if property.IsUnicode().HasValue then
                    let value =
                        if property.IsUnicode().Value then "" else "false"
                ...
                if isSuccess then
                    if defaultValue = box DBNull.Value then
                        ...
                        lines.Add
                            ".HasDefaultValue({code.UnknownLiteral(defaultValue)})"
        ...
        member this.WriteCode
            ...
            let mutable finalNamespaces =
                ...
                |> Seq.sortBy(fun n ->
                    (match n with
                     | "System" -> 1
                     | x when x.StartsWith("System", StringComparison.Ordinal) ->
                         2
                     | x when
            ...
            then
                finalNamespaces <-
                    finalNamespaces |> Seq.append [ modelNamespace ]
