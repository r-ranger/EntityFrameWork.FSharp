...
type internal AttributeWriter(name : string) =
    ...
    member _.AddParameter p = parameters.Add p
...
type FSharpEntityTypeGenerator
            ...
            if t.GetGenericTypeDefinition() = typedefof<Nullable<_>> then
                ...
                let genericTypeArguments =
                    String.Join(
                        ...
                        |> Seq.map(fun t' ->
                            getTypeName scaffoldNullableColumnsAs t'
                        )
                    )
    ...
    let generateRequiredAttribute (p : IProperty) =
        ...
        if
            ...
            && (p.IsPrimaryKey() |> not)
        then
            (nameof RequiredAttribute |> createAttributeQuick) |> Some
        else
            None
    ...
    let generateKeyAttribute (property : IProperty) =
        if notNull(property.FindContainingPrimaryKey()) then
            (nameof KeyAttribute |> createAttributeQuick) |> Some
        else
            None

    let generateKeylessAttribute (entityType : IEntityType) =
        if isNull(entityType.FindPrimaryKey()) then
            (nameof KeylessAttribute |> createAttributeQuick) |> Some
        else
            None
    ...
    let generateTableAttribute (entityType : IEntityType) =
        ...
        let tableAttributeNeeded =
            ...
            && (schemaParameterNeeded
                || notNull tableName && tableName <> entityType.GetDbSetName())
        ...
        let attributes =
            ...
            |> Seq.map(fun index ->
                ...
                if annotations.Count = 0 then
                    ...
                    index.Properties
                    |> Seq.iter(fun p ->
                        indexAttribute.AddParameter $"nameof({p.Name})"
                    )

                    if notNull index.Name then
                        indexAttribute.AddParameter
                            $"Name = {code.Literal(index.Name)}"

                    if index.IsUnique then
                        indexAttribute.AddParameter
                            $"IsUnique = {code.Literal(index.IsUnique)}"
    ...
    let generateInversePropertyAttribute (navigation : INavigation) =
        ...
        then
            ...
            let nameMatches =
                ...
                |> Seq.exists(fun m ->
                    m.Name = inverseNavigation.DeclaringEntityType.Name
                )
    ...
    let generateConstructor (entityType : IEntityType) =
        ...
        let collectionNavigations =
            entityType.GetNavigations() |> Seq.filter(_.IsCollection)
        ...
        let annotations =
            stringBuilder {
                ...
                for a in
                    ...
                    a.Arguments
                    |> Seq.iter(fun arg ->
                        attributeWriter.AddParameter(code.UnknownLiteral arg)
                    )
    ...
    let generateProperties
        ...
        let props =
            ...
            |> Seq.sortBy(_.GetColumnOrder().GetValueOrDefault(-1))
    ...
    let generateNavigationProperties
        ...
        let sortedNavigations =
            ...
            |> Seq.sortBy(fun n ->
                ((if n.IsOnDependent then 0 else 1),
                 (if n.IsCollection then 1 else 0))
            )
    ...
    let generateSkipNavigateTypeEntry
        (n : ISkipNavigation)
        (useDataAnnotations : bool)
        =
