...
type FSharpMigrationOperationGenerator(code : ICSharpHelper) =
    ...
    let toOnedimensionalArray firstDimension (a : obj[,]) =
        Array.init
            a.Length
            (fun i -> if firstDimension then a[i, 0] else a[0, i])
    ...
    let annotations includeIgnore (annotations : Annotation seq) =
        ...
        if lines |> Seq.isEmpty then
            ...
            let tail =
                ...
                |> Seq.tail
                |> Seq.map(fun l ->
                    if includeIgnore && l = last then l + " |> ignore" else l
                )
    ...
    let generateAddForeignKeyOperation (op : AddForeignKeyOperation) =
        ...
        stringBuilder {
            ...
            indent {
                ...
                writeParameterIfTrue
                    (op.OnUpdate <> ReferentialAction.NoAction)
                    "onUpdate"
                    op.OnUpdate

                writeParameterIfTrue
                    (op.OnDelete <> ReferentialAction.NoAction)
                    "onDelete"
                    op.OnDelete
            }
    ...
    let generateAddUniqueConstraintOperation
    ...
    let generateAlterColumnOperation (op : AlterColumnOperation) =
        stringBuilder {
            ...
            indent {
                ...
                writeParameterIfTrue
                    (op.OldColumn.ColumnType |> notNull)
                    "oldType"
                    op.OldColumn.ColumnType

                writeNullableParameterIfValue
                    "oldUnicode"
                    op.OldColumn.IsUnicode

                writeNullableParameterIfValue
                    "oldMaxLength"
                    op.OldColumn.MaxLength

                writeParameterIfTrue
                    op.OldColumn.IsRowVersion
                    "oldRowVersion"
                    true
                ...
                writeOptionalParameter
                    "oldDefaultValueSql"
                    op.OldColumn.DefaultValueSql

                writeOptionalParameter
                    "oldComputedColumnSql"
                    op.OldColumn.ComputedColumnSql

                writeOptionalParameter
                    "oldDefaultValue"
                    op.OldColumn.DefaultValue
    ...
    let generateAlterSequenceOperation (op : AlterSequenceOperation) =
        stringBuilder {
            ...
            indent {
                ...
                writeParameterIfTrue
                    (op.OldSequence.IncrementBy <> 1)
                    "oldIncrementBy"
                    op.OldSequence.IncrementBy

                writeNullableParameterIfValue
                    "oldMinValue "
                    op.OldSequence.MinValue

                writeNullableParameterIfValue
                    "oldMaxValue "
                    op.OldSequence.MaxValue
    ...
    let generateCreateTableOperation (op : CreateTableOperation) =
                ...
                indent {
                    ...
                    indent {
                        ...
                        if c.DefaultValueSql |> notNull then
                            sprintf
                                ", defaultValueSql = %s"
                                (code.Literal c.DefaultValueSql)
                        elif c.ComputedColumnSql |> notNull then
                            sprintf
                                ", computedColumnSql = %s"
                                (code.Literal c.ComputedColumnSql)
                        elif c.DefaultValue |> notNull then
                            sprintf
                                ", defaultValue = %s"
                                (code.UnknownLiteral c.DefaultValue)
                    }
                }
        ...
        let writeColumns =
            stringBuilder {
                ...
                indent {
                    op.Columns |> Seq.filter notNull |> Seq.map writeColumn
                }
    ...
    let generateDropUniqueConstraintOperation
            ...
            seq {
                ...
                let valuesArray =
                    if length0 = 1 && length1 = 1 then
                        sprintf
                            "value = %s :> obj"
                            (op.Values[0, 0] |> code.UnknownLiteral)
                    elif length0 = 1 then
                        sprintf
                            "values = %s"
                            (op.Values
            ...
            seq {
                ...
                if op.KeyColumns.Length = 1 then
                    yield
                        sprintf
                            "keyColumn = %s, "
                            (op.KeyColumns[0] |> code.Literal)
                else
                    yield
                        sprintf
                            "keyColumns = %s, "
                            (op.KeyColumns |> code.Literal)
                ...
                if length0 = 1 && length1 = 1 then
                    yield
                        sprintf
                            "keyValue = %s"
                            (op.KeyValues[0, 0] |> code.UnknownLiteral)
                elif length0 = 1 then
                    yield
                        sprintf
                            "keyValues = %s"
                    ...
                    yield
                        sprintf "keyValues = %s" (op.KeyValues |> code.Literal)
            }
    ...
    let generateUpdateDataOperation (op : UpdateDataOperation) =
        let parameters =
            seq {
                ...
                if op.Columns.Length = 1 then
                    yield
                        sprintf "column = %s, " (op.Columns[0] |> code.Literal)
                else
                    yield sprintf "columns = %s, " (op.Columns |> code.Literal)
    ...
    let generateDropCheckConstraintOperation
        (op : DropCheckConstraintOperation)
        =
        stringBuilder {
            ".DropCheckConstraint("
    ...
    let generateOperation builderName (op : MigrationOperation) =
        let result =
            ...
            | :? AddForeignKeyOperation as op' ->
                op' |> generateAddForeignKeyOperation
            | :? AddPrimaryKeyOperation as op' ->
                op' |> generateAddPrimaryKeyOperation
            | :? AddUniqueConstraintOperation as op' ->
                op' |> generateAddUniqueConstraintOperation
            | :? AlterColumnOperation as op' ->
                op' |> generateAlterColumnOperation
            | :? AlterDatabaseOperation as op' ->
                op' |> generateAlterDatabaseOperation
            | :? AlterSequenceOperation as op' ->
                op' |> generateAlterSequenceOperation
            | :? AlterTableOperation as op' ->
                op' |> generateAlterTableOperation
            | :? CreateIndexOperation as op' ->
                op' |> generateCreateIndexOperation
            | :? EnsureSchemaOperation as op' ->
                op' |> generateEnsureSchemaOperation
            | :? CreateSequenceOperation as op' ->
                op' |> generateCreateSequenceOperation
            | :? CreateTableOperation as op' ->
                op' |> generateCreateTableOperation
            | :? DropColumnOperation as op' ->
                op' |> generateDropColumnOperation
            | :? DropForeignKeyOperation as op' ->
                op' |> generateDropForeignKeyOperation
            | :? DropIndexOperation as op' -> op' |> generateDropIndexOperation
            | :? DropPrimaryKeyOperation as op' ->
                op' |> generateDropPrimaryKeyOperation
            | :? DropSchemaOperation as op' ->
                op' |> generateDropSchemaOperation
            | :? DropSequenceOperation as op' ->
                op' |> generateDropSequenceOperation
            | :? DropTableOperation as op' -> op' |> generateDropTableOperation
            | :? DropUniqueConstraintOperation as op' ->
                op' |> generateDropUniqueConstraintOperation
            | :? RenameColumnOperation as op' ->
                op' |> generateRenameColumnOperation
            | :? RenameIndexOperation as op' ->
                op' |> generateRenameIndexOperation
            | :? RenameSequenceOperation as op' ->
                op' |> generateRenameSequenceOperation
            | :? RenameTableOperation as op' ->
                op' |> generateRenameTableOperation
            | :? RestartSequenceOperation as op' ->
                op' |> generateRestartSequenceOperation
            | :? SqlOperation as op' -> op' |> generateSqlOperation
            | :? InsertDataOperation as op' ->
                op' |> generateInsertDataOperation
            | :? DeleteDataOperation as op' ->
                op' |> generateDeleteDataOperation
            | :? UpdateDataOperation as op' ->
                op' |> generateUpdateDataOperation
            | :? AddCheckConstraintOperation as op' ->
                op' |> generateAddCheckConstraintOperation
            | :? DropCheckConstraintOperation as op' ->
                op' |> generateDropCheckConstraintOperation
            | _ ->
                op |> invalidOp(op.GetType() |> DesignStrings.UnknownOperation) // The failure case
