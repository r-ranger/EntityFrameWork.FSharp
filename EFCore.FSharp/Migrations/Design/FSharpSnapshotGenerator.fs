...
type FSharpSnapshotGenerator
    ...
    let generateAnnotations
        ...
        let fluentApiCalls =
            annotationCodeGenerator.GenerateFluentApiCalls(
                annotatable,
                annotations
            )
                ...
                if defaultValue <> (box DBNull.Value) then
                    ...
                    let typeMap =
                        ...
                        then
                            ...
                            (mappingSource.FindMapping(property)
                            :> CoreTypeMapping)
                            |> Option.ofObj
    ...
    let genPropertyAnnotations propertyBuilderName (property : IProperty) =
        ...
        let columnType (p : IProperty) =
            let columnType =
                p.GetColumnType() ?= mappingSource.GetMapping(p).StoreType
    ...
    let generateIndex (entityTypeBuilderName : string) (idx : IIndex) =
        ...
        let indexParams =
            if isNull idx.Name then
                String.Join(
                    ", ",
                    (idx.Properties |> Seq.map(fun p -> p.Name |> code.Literal))
                )
            else
                sprintf
                    "[| %s |], %s"
    ...
    let generateSequence (builderName : string) (sequence : ISequence) =
        ...
        let lines =
            seq {
                if
                    sequence.StartValue <> (Sequence.DefaultStartValue |> int64)
                then
    ...
    member this.generateCheckConstraints
        (builderName : string)
    ...
    member this.generateEntityTypeAnnotations
        ...
        let getAnnotationValue
            (annotation : IAnnotation)
            (defaultValue : unit -> string)
            =
        ...
        then
            ...
            if notNull tableName || notNull tableNameAnnotation then
                ...
                if
                    isNull tableName
                    && (isNull schemaAnnotation || isNull schema)
                then
                    sb.Append(
                        sprintf "(string %s)" (code.UnknownLiteral tableName)
                    )
                    |> ignore
                else
                ...
                if
                    notNull schema
                    || (notNull schemaAnnotation && notNull tableName)
                then
                    if
                        isNull schema
                    ...
                    then
                        sb.Append(
                            sprintf ", (string %s)" (code.UnknownLiteral schema)
                        )
                        |> ignore
        ...
        then
            ...
            if notNull viewName then
                ...
                if annotationAndValueNotNull viewSchemaAnnotation then
                    let viewSchemaAnnotationValue =
                        viewSchemaAnnotation.Value :?> string
            ...
            if annotationAndValueNotNull discriminatorPropertyAnnotation then
                let discriminatorProperty =
                    entityType.FindProperty(
                        discriminatorPropertyAnnotation.Value :?> string
                    )
    ...
    member private this.generateForeignKeyAnnotations
        entityTypeBuilderName
        (fk : IForeignKey)
        =
    ...
    member private this.generateForeignKey
        entityTypeBuilderName
        (fk : IForeignKey)
        =
        ...
        if fk.IsUnique && (not fk.IsOwnership) then
            ...
            if fk.PrincipalKey <> fk.PrincipalEntityType.FindPrimaryKey() then
                lines.Add
                    $".HasPrincipalKey({literalPropNames fk.PrincipalKey.Properties})"
    ...
    member private this.generateForeignKeys
        entityTypeBuilderName
        (foreignKeys : IForeignKey seq)
        =
    ...
    member private this.generateRelationships
        ...
        let ownerships =
            ...
            |> Seq.filter(_.IsOwnership)

        stringBuilder {
            this.generateForeignKeys
                entityTypeBuilderName
                (getDeclaredForeignKeys entityType)
    ...
    member private this.generateEntityType
        ...
        let entityTypeBuilderName =
            if builderName.StartsWith("b", StringComparison.Ordinal) then
                ...
                match
                    builderName.Length > 1, Int32.TryParse(builderName[1..])
                with
        ...
        stringBuilder {
            ...
            indent {
                ...
                this.generateEntityTypeAnnotations
                    entityTypeBuilderName
                    entityType
                ...
                ownerNav
                |> Option.map(fun _ ->
                    this.generateRelationships entityTypeBuilderName entityType
                )

                this.generateData
                    entityTypeBuilderName
                    (entityType.GetProperties())
                    (entityType |> getData true)
            }
    ...
    member private this.generateEntityTypeNavigations
        ...
        let navigations =
            ...
            |> Seq.filter(fun n ->
                (not n.IsOnDependent) && (not n.ForeignKey.IsOwnership)
            )
    ...
    member private this.generateEntityTypes
        builderName
        (entities : IEntityType seq)
        =
    ...
    member _.generatePropertyAnnotations property =
        genPropertyAnnotations property
