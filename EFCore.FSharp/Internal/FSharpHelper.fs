...
type FSharpHelper(relationalTypeMappingSource : IRelationalTypeMappingSource) =
    ...
    member private this.ReferenceFullName (t : Type) useFullName =
        ...
        | _ ->
            if t |> isNullableType then
                ...
                sprintf
                    "%s option"
                    (this.ReferenceFullName (t |> unwrapOptionType) useFullName)
            else
                ...
                if t.IsArray then
                    builder
                        .Append(
                            this.ReferenceFullName (t.GetElementType()) false
                        )
                        .Append("[")
                    |> ignore
    ...
    member private this.literalDateTimeOffset(value : DateTimeOffset) =
        sprintf
            "DateTimeOffset(%s, %s)"
            (value.DateTime |> this.literalDateTime)
            (value.Offset |> this.literalTimeSpan)
    ...
    member private this.literalArray2D(values : obj[,]) =
        ...
        let rowContents =
            ...
            |> Seq.map(fun i ->
                ...
                let entries =
                    row' |> Seq.map this.unknownLiteral |> Seq.map literalAsObj
    ...
    member private this.handleExpression
        ...
        | ExpressionType.NewArrayInit ->
            ...
            this.handleList
                (expression :?> NewArrayExpression).Expressions
                true
                sb
            |> ignore
        ...
        | ExpressionType.Call ->
            ...
            if callExpr.Method.IsStatic then
                sb.Append(
                    this.ReferenceFullName callExpr.Method.DeclaringType true
                )
                |> ignore
    ...
    member private this.handleScope
    ...
    member private this.IdentifierWithScope
        (name : string)
        (scope : ICollection<string>)
        =
    ...
    member private this.buildFragment
        ...
        if typeQualified then
            ...
            builder.Append
                $"%s{fragment.DeclaringType}.%s{fragment.Method}(%s{instanceIdentifier}"
            |> ignore
    ...
    member private this.buildNestedFragment
        (
            n : NestedClosureCodeFragment,
            indent : int
        )
        =
        let builder = IndentedStringBuilder()
        ...
        let lines =
            n.MethodCalls
            |> Seq.map(fun mc ->
                this.buildFragment(mc, false, n.Parameter, indent + 1)
            )
            ...
            | _ ->
                ...
                if isNull mapping then
                    ...
                    if handled then
                        ...
                        let args =
                            (expression.ToString(),
                             (displayName literalType false false))
    ...
    interface ICSharpHelper with
        ...
        member this.Literal(values : 'T[], vertical : bool) : string =
            ...
            this.literalList
                (values |> Seq.cast<obj> |> ResizeArray)
                vertical
                isObjType
        ...
        member this.Namespace(name : string[]) : string =
            ...
            let ns =
                ...
                |> Array.map(fun t ->
                    (this :> ICSharpHelper).Identifier(t, null)
                )
                |> Array.filter(String.IsNullOrEmpty >> not)
                |> join
