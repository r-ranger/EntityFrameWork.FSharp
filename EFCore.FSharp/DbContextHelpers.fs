...
let private transform (a : obj) =
    ...
    match a with
    | :? seq<obj> as s ->
        Array.ofSeq s |> Array.map(fun i -> box i) |> Composite
    | _ -> Single a
...
let tryFindEntity<'a when 'a : not struct>
    (ctx : DbContext)
    (key : obj)
    : 'a option
    =
...
let findEntityAsync<'a when 'a : not struct>
    (ctx : DbContext)
    (key : obj)
    : Async<'a>
    =
...
let findEntityTaskAsync<'a when 'a : not struct>
    (ctx : DbContext)
    (key : obj)
    : ValueTask<'a>
    =
...
let tryFindEntityAsync<'a when 'a : not struct>
    (ctx : DbContext)
    (key : obj)
    : Async<'a option>
    =
...
let tryFindEntityTaskAsync<'a when 'a : not struct>
    (ctx : DbContext)
    (key : obj)
    : Task<'a option>
    =
    ...
    result
        ...
        .ContinueWith(fun (t : Task<'a>) ->
            if isNull(box t.Result) then None else Some t.Result
        )
...
let updateEntityRange
    (ctx : DbContext)
    (key : 'a -> 'b)
    (entities : 'a seq when 'a : not struct)
    =
    entities |> Seq.map(fun e -> updateEntity ctx key e)

let updateEntityRangeAsync
    (ctx : DbContext)
    (key : 'a -> 'b)
    (entities : 'a seq when 'a : not struct)
    =
...
let addEntityTaskAsync' (ctx : #DbContext) (entity : 'a when 'a : not struct) =
    ctx.Set<'a>().AddAsync(entity).AsTask()
...
let addEntityRange (ctx : #DbContext) (entities : 'a seq when 'a : not struct) =
    addEntityRange' ctx entities

let addEntityRangeAsync'
    (ctx : #DbContext)
    (entities : 'a seq when 'a : not struct)
    =
...
let attachEntityRange
    ...
    (entities : 'a seq when 'a : not struct)
    =
    attachEntityRange' ctx entities
...
let removeEntityRange
    ...
    (entities : 'a seq when 'a : not struct)
    =
    removeEntityRange' ctx entities
...
let tryFilterFirstAsync predicate (dbSet : #IQueryable<_>) =
    async {
        ...
        let! ret =
            dbSet.FirstOrDefaultAsync(predicate = pred) |> Async.AwaitTask
...
type IQueryable<'T> with
    ...
    member this.TryFirstAsync expr =
        async {
            let! ret =
                this.FirstOrDefaultAsync(predicate = expr) |> Async.AwaitTask
